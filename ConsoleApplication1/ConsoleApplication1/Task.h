...
HCRYPTKEY genKeySession(HCRYPTPROV hCryptProv, LPTSTR pszNameProv, DWORD type, ALG_ID alg_id) {
    LPSTR pszUserName;
    DWORD dwUserNameLen;
...
BYTE* ReadBlobFile(DWORD &bufLen, LPCSTR nameFile) {
    ...
    if ((fopen_s(&EncryptedKeyBlob, nameFile, "rb"))) {
...
void Task4(HCRYPTPROV &hCryptProv, LPTSTR pszNameProv, DWORD type, LPCWSTR nameContainer) {  // handle for a cryptographic
    ...
    else
    {
        ...
        if (CryptAcquireContext(
            ...
            0))
        {
            printf("A exist container has been opened.\n");
        }
        else
        {
            return;
    ...
    if (CryptGetUserKey(
        ...
        &hKey))                         // Дескриптор ключа
    {
        printf("A exchenge key is available.\n");
    }
    else
    {
        printf("No signature key is available.\n");
        ...
        // Создание подписанной ключевой пары. 
        printf("The exchenge key does not exist.\n");
        printf("Creating a exchenge key pair...\n");
        ...
        if (!CryptGenKey(
            ...
            &hKey))
        {
            printf("Error occurred creating a exchenge key.\n");
            exit(1);
        }
        printf("Created a exchenge key pair.\n");
